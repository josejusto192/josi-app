...
/*
  ...
  Brand: Josiane Szewczuk Ferraz — criadora de conteúdo rural/lifestyle
  Palette: terracota #C9826B · verde-sálvia #8A9E7B · dourado #D4A96A
           bege #F5EDE3 · off-white #FDF8F3 · marrom #4A2E22
  Fonts:   Playfair Display (display) · DM Sans (body) · Cormorant Garamond italic (callout)
  Tone:    warm · intimate · "amiga/irmã" · 1ª pessoa · informal PT-BR
*/
...
export default function LandingPage() {
  ...
  const Logo = () => (
    <svg viewBox="0 0 200 44" width="130" height="40" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="0" y="38" fontFamily="'Cormorant Garamond',Georgia,serif" fontSize="46" fontWeight="500" fontStyle="italic" fill="url(#lg)">J</text>
  ...
  const cats = [
    { emoji:'🛍️', name:'Comprinhas',   color:'#F0D5C8', border:'#E0BBA8', desc:'Hauls Shopee, Shein e tudo que eu amei' },
    { emoji:'💑', name:'Casal',         color:'#FDE8D8', border:'#F0CDB8', desc:'Momentos com o Jeck e nossa vida a dois' },
    { emoji:'👨‍👩‍👧', name:'Família',       color:'#E8F0E0', border:'#C8DDB8', desc:'Vida com a Isabeli e tudo sobre maternidade' },
    { emoji:'🐾', name:'Pets',          color:'#F5F0E0', border:'#E0D8B8', desc:'Nossos animais e a vida no campo' },
    { emoji:'📚', name:'Cursos',         color:'#F0E8D8', border:'#DCCAB0', desc:'Conteúdo exclusivo para você crescer' },
    { emoji:'✨', name:'Skincare',       color:'#F8E8F0', border:'#E8C8DC', desc:'Rotina de pele — simples e que funciona' },
    { emoji:'🌿', name:'Chácara',        color:'#E0EDD8', border:'#BCDAB0', desc:'Vida no campo, natureza e liberdade' },
    { emoji:'🍳', name:'Receitas',       color:'#FDE8D0', border:'#F0CCAC', desc:'Tudo que saiu da minha cozinha hoje' },
    { emoji:'🤱', name:'Maternidade',    color:'#F8EAE8', border:'#E8C8C4', desc:'O que ninguém te conta sobre ser mãe' },
  ]
...
.lp-nav {
  ...
  background:rgba(253,248,243,0.90); backdrop-filter:blur(12px);
...
.btn {
  ...
  border-radius:100px; border:none; cursor:pointer; transition:all .2s; white-space:nowrap;
}
.btn-primary {
  background:linear-gradient(135deg,#C9826B,#D4A96A); color:#FDF8F3;
  box-shadow:0 4px 16px rgba(201,130,107,.35);
  font-size:14px; padding:12px 26px;
}
.btn-primary:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(201,130,107,.45); }
.btn-outline {
  background:transparent; border:1.5px solid #C9826B; color:#C9826B;
...
.btn-white {
  background:#FDF8F3; color:#4A2E22;
  box-shadow:0 4px 14px rgba(74,46,34,.15);
  font-size:14px; padding:12px 26px;
}
...
.hero-badge {
  ...
  background:#FDF8F3; border:1px solid #E8D8CC;
...
.hero-badge span { width:6px;height:6px;border-radius:50%;background:linear-gradient(135deg,#C9826B,#D4A96A); }
.hero-h1 {
...
.hero-h1 em {
  font-style:italic; font-weight:400; color:#C9826B;
}
.hero-callout {
  font-family:'Cormorant Garamond',Georgia,serif;
  font-style:italic; font-size:clamp(18px,2.8vw,26px);
  font-weight:400; color:#8A6A5A; max-width:560px;
...
.hero-avatars {
  ...
  font-size:13px; font-weight:400; color:#8A6A5A;
}
...
/* ── about strip ── */
.about-strip {
  background:linear-gradient(135deg,#C9826B 0%,#D4A96A 100%);
  padding:clamp(40px,6vw,64px) clamp(20px,6vw,64px);
...
.cat-emoji { font-size:26px; flex-shrink:0; margin-top:1px; }
...
.testi-card {
  ...
  padding:26px 22px; transition:transform .22s, box-shadow .22s;
}
.testi-card:hover { transform:translateY(-3px); box-shadow:0 8px 24px rgba(74,46,34,.10); }
.testi-stars { color:#D4A96A; font-size:13px; letter-spacing:2px; margin-bottom:12px; }
.testi-text {
  font-family:'Cormorant Garamond',Georgia,serif;
  font-style:italic; font-size:17px; font-weight:400;
  color:#4A2E22; line-height:1.6; margin-bottom:18px;
}
...
.fq-a { max-height:0;overflow:hidden;transition:max-height .36s ease,padding .24s;padding:0 0;font-size:14px;font-weight:300;color:#8A6A5A;line-height:1.75; }
.fq.open .fq-a { max-height:220px;padding-bottom:18px; }

/* ── cta ── */
.cta-sec {
  background:linear-gradient(135deg,#4A2E22 0%,#6A3E2A 100%);
  padding:clamp(64px,10vw,100px) clamp(20px,6vw,64px);
  text-align:center; position:relative; overflow:hidden;
}
.cta-sec::before {
  content:''; position:absolute; inset:0; pointer-events:none;
  background:radial-gradient(ellipse 70% 60% at 50% 50%,rgba(201,130,107,.18),transparent 60%);
}
.cta-h {
  font-family:'Playfair Display',Georgia,serif;
  font-size:clamp(32px,6vw,64px); font-weight:600;
  line-height:1.15; letter-spacing:-.015em;
  color:#FDF8F3; margin-bottom:18px;
}
.cta-h em { font-style:italic;font-weight:400;color:#D4A96A; }
.cta-callout {
  font-family:'Cormorant Garamond',Georgia,serif;
  font-style:italic; font-size:clamp(17px,2.5vw,22px);
  color:rgba(253,248,243,.6); max-width:480px; margin:0 auto 36px;
  line-height:1.65;
}

/* ── footer ── */
.footer {
  background:#3A2218; padding:28px clamp(20px,6vw,64px);
...
.up:nth-child(5){transition-delay:.32s}
.up:nth-child(6){transition-delay:.4s}
.up:nth-child(7){transition-delay:.48s}
.up:nth-child(8){transition-delay:.56s}
.up:nth-child(9){transition-delay:.64s}
      ...
      <div className="lp">
        ...
        <section className="hero">
          ...
          <div className="hero-badge">
            <span /> Aplicativo oficial da Josi
          </div>
          ...
          <div className="hero-btns">
            <Link href="/auth/login" className="btn btn-primary btn-lg">Quero acessar agora ✨</Link>
            <a href="#conteudo" className="btn btn-outline btn-lg">Ver o conteúdo</a>
          </div>
        ...
        <section className="sec" id="conteudo">
          <div className="sec-inner">
            ...
            <div className="cat-grid">
              {cats.map(c => (
                <div key={c.name} className="cat-card up" style={{ background:c.color, borderColor:c.border }}>
            ...
            <div className="price-grid">
              ...
              <div className="price-card up">
                ...
                <ul className="price-list">
                  {['Acesso à comunidade','Conteúdo introdutório','Perfil personalizado','Publicações e interações'].map(i => (
                    <li key={i}><span className="chk"><svg viewBox="0 0 8 8" fill="none"><path d="M1 4l2 2 4-3" stroke="#C9826B" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span>{i}</li>
                  ))}
                </ul>
              ...
              <div className="price-card price-featured up">
                ...
                <ul className="price-list">
                  {[
                    ...
                    'Skincare — rotina completa',
                    'Conteúdo de chácara e pets',
                    'Novos conteúdos toda semana',
                  ].map(i => (
                    <li key={i}><span className="chk"><svg viewBox="0 0 8 8" fill="none"><path d="M1 4l2 2 4-3" stroke="#FDF8F3" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg></span>{i}</li>
                  ))}
                </ul>
          ...
          <div className="sec-inner" style={{ textAlign:'center' }}>
            ...
            <div className="faq-list" style={{ textAlign:'left' }}>
              {[
                ...
                { q:'O conteúdo gratuito vale a pena?', a:'Claro! Você já tem acesso à comunidade, ao perfil e ao conteúdo de entrada. O Premium desbloqueia os cursos, tutoriais completos e o exclusivo.' },
              ].map(({ q, a }) => (

...
export default async function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      {/* Back */}
      <Link href="/admin/usuarios" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: '#8A6A5A', textDecoration: 'none', fontSize: 13, marginBottom: 20 }}>
        ← Voltar para usuárias
      </Link>

      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg,#4A2E22,#7A4A32)', borderRadius: 20, padding: '24px 28px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(212,169,106,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, color: '#D4A96A', fontWeight: 600 }}>
            {(profile.nome ?? 'U')[0].toUpperCase()}
          </span>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 24, fontWeight: 600, color: '#FDF8F3' }}>{profile.nome ?? 'Sem nome'}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, textAlign: 'center' }}>
          {[
            ...
            { label: 'Peso inicial', value: profile.peso_inicial ? `${profile.peso_inicial}kg` : '—', color: '#FDF8F3' },
            { label: 'Perdeu',       value: weightLost !== null && weightLost > 0 ? `−${weightLost}kg` : '—', color: '#8A9E7B' },
          ].map(s => (
            <div key={s.label}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: '#FDF8F3', borderRadius: 18, padding: 22, boxShadow: '0 2px 10px rgba(74,46,34,0.08)' }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 600, color: '#4A2E22', marginBottom: 16 }}>Perfil (onboarding)</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <div key={row.label} style={{ display: 'flex', gap: 8 }}>
                <span style={{ fontSize: 12, color: '#8A6A5A', minWidth: 90, fontWeight: 600 }}>{row.label}</span>
                <span style={{ fontSize: 13, color: '#4A2E22', flex: 1 }}>{String(row.value)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Hábitos */}
          <div style={{ background: '#FDF8F3', borderRadius: 18, padding: 22, boxShadow: '0 2px 10px rgba(74,46,34,0.08)' }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 600, color: '#4A2E22', marginBottom: 14 }}>Hábitos (últimos 30 dias)</div>
            {[
              { label: '💧 Água',     value: habStats.agua,     color: '#6BA3BE' },
              { label: '🥩 Proteína', value: habStats.proteina, color: '#C9826B' },
              { label: '👟 Passos',   value: habStats.passos,   color: '#8A9E7B' },
              { label: '🏋️ Treino',  value: habStats.treino,   color: '#D4A96A' },
            ].map(h => (
              <div key={h.label} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <span style={{ fontSize: 12, color: '#4A2E22' }}>{h.label}</span>
            ...
            {hab.length === 0 && <div style={{ fontSize: 13, color: '#B89B8C' }}>Sem registros ainda</div>}
          </div>

          {/* Medidas */}
          {(measures ?? []).length > 0 && (
            <div style={{ background: '#FDF8F3', borderRadius: 18, padding: 22, boxShadow: '0 2px 10px rgba(74,46,34,0.08)' }}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 600, color: '#4A2E22', marginBottom: 14 }}>Último registro de medidas</div>
              <div style={{ fontSize: 11, color: '#8A6A5A', marginBottom: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {[
                  ...
                  <div key={m.label} style={{ background: '#F5EDE3', borderRadius: 10, padding: '10px 12px' }}>
                    <div style={{ fontSize: 10, color: '#8A6A5A', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{m.label}</div>
                    <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: '#C9826B', fontWeight: 600 }}>{m.value} <span style={{ fontSize: 12 }}>{m.unit}</span></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Desafio grid */}
      <div style={{ background: '#FDF8F3', borderRadius: 18, padding: 22, boxShadow: '0 2px 10px rgba(74,46,34,0.08)', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 600, color: '#4A2E22' }}>Progresso no desafio</div>
            {enrollment && (
              <div style={{ fontSize: 12, color: '#8A6A5A', marginTop: 2 }}>
                Iniciado em {new Date(enrollment.start_date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long' })}
              </div>
            )}
          </div>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 600, color: '#C9826B' }}>{doneDays}/21</div>
        </div>
          ...
          {Array.from({ length: 21 }, (_, i) => {
            ...
            return (
              <div key={day} title={done ? `Dia ${day} ✓ ${prog?.completed_at ? new Date(prog.completed_at).toLocaleDateString('pt-BR') : ''}` : `Dia ${day}`}
                style={{ aspectRatio: '1', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? '#8A9E7B' : '#F0D5C8', border: done ? 'none' : '1px solid #E8D8CC' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: done ? '#FDF8F3' : '#C4B0A8' }}>{done ? '✓' : day}</span>
              </div>
            )
          })}
        </div>
      ...
      {(progress ?? []).filter((p: { anotacao: string | null }) => p.anotacao).length > 0 && (
        <div style={{ background: '#FDF8F3', borderRadius: 18, padding: 22, boxShadow: '0 2px 10px rgba(74,46,34,0.08)' }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 600, color: '#4A2E22', marginBottom: 16 }}>📝 Anotações da aluna</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {(progress ?? []).filter((p: { anotacao: string | null }) => p.anotacao).map((p: { day: number; anotacao: string | null; completed_at: string | null }) => (
              <div key={p.day} style={{ background: '#F5EDE3', borderRadius: 12, padding: '12px 14px' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#C9826B', marginBottom: 4 }}>Dia {p.day}</div>
                <div style={{ fontSize: 13, color: '#4A2E22', lineHeight: 1.5 }}>{p.anotacao}</div>
              </div>
            ))}
          </div>
        </div>
